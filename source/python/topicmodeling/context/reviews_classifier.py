...
class ReviewsClassifier():
    ...
    def __init__(self):
        pass
        # self.time_words = filter(
        #     lambda s: s.lexname() in {'noun.time'}, wordnet.all_synsets('n'))
        # self.time_words = {
        #     lemma.name() for s in self.time_words for lemma in s.lemmas()}
        # print('time words length', len(self.time_words))

    # def get_time_words(self, text):
    #     words = get_words(text)
    #     return self.time_words.intersection(words)

    def get_review_metrics(self, review):
        """
...
def main():
    ...
    for index in range(len(my_reviews)):
        my_metrics[index] = classifier.get_review_metrics(my_reviews[index])
        # print(my_metrics[index])

    normalize_matrix_by_columns(my_metrics)
