...
def save_lda_doc_weights(out_path, W ):
        """
        Save a lda document weight matrix
        """
        joblib.dump(W, out_path )

def load_lda_doc_weights( in_path):
        """
        Load a lda document weight matrix
        """
        W = joblib.load( in_path )
        return(W)


# --------------------------------------------------------------

def write_table( out_path, tab, delimiter = ',' ):
    """ 
    Utility function to write a Prettytable table to a plain text output file, with the specified delimiter.
    """
    import csv
